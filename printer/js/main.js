...
(function getxhr() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200){
            ...
            for (var i=0; i<jsonobj.pri.length ; i++) {
                for (var j=0; j<jsonobj.pri[i].length ; j++){
                    var crediv = document.createElement("div");
                    var creimg = document.createElement("IMG");
                    getdiv.appendChild(crediv);
                    crediv.setAttribute("class" , "div1child1mob");
                    creimg.src = jsonobj.pri[i][j].img;
                    console.log(creimg);
                    crediv.insertBefore( creimg , crediv.childNodes[0]);
                    creimg.setAttribute("class" , "div1child1imgmob");
                    crediv.innerHTML += "<p onclick=product(this) id="+[1,i,+" "+j]+">"+jsonobj.pri[i][j].name+"</p>"+"<h4 >"
                    +jsonobj.pri[i][j].price+"</h4>" +"<button  class=btnmob onclick=addtocart(this) id="+[1,i,+" "+j]+">Add to Cart</button>" ;
                }
            }
        }
    }
